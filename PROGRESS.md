...
## Remaining work
- [ ] Build the Activity Data schema split — raw/converted value and unit
      pairs, plus new evidence_link and reviewer fields (v4.0 revision)
- [ ] Build the hardcoded emission_source → conversion_factor lookup table
      (every factor placeholder 1.0, marked TBD) and the conversion
      computation, with the Data Review fallback for an unrecognized raw
      unit on CSV import (v4.0 revision)
- [ ] Build the Facility Reporting Period form and its own session table
      (facility, reporting_year, facility_country, production_volume,
      annual_revenue) (v4.0 revision)
- [ ] Build the placeholder Dashboard nav tab — active link to a placeholder
      URL, to be swapped once Tool B deploys (v4.0 revision)
- [ ] Split the export into two separate CSV downloads — Activity Data and
      Facility Reporting Period (v4.0 revision)
- [ ] Local test pass covering the v4.0 additions, including the
      unrecognized-unit-on-import edge case (v4.0 revision)
- [ ] Acceptance criteria pass — verify every criterion in spec Section 13
      before deploy (v4.0 revision)
...
## Known issues
- Spec revised to v4.0 on 2026-07-14 — CLAUDE.md regenerated by Project Governor.
